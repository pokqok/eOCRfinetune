...
직접 파인튜uning하는 방법
1. 데이터셋 준비
학습 및 검증에 사용할 이미지와 라벨 파일(labels.csv)을 각각 data/train과 data/validation 폴더에配置합니다. 데이터 구조는 파인튜닝 스크립트가 요구하는 형식을 따라야 합니다.
...
감사의 말
이 프로젝트는 강력하고 유연한 EasyOCR 라이브러리를 기반으로 제작되었습니다.
...
수많은 py들은 파인튜닝시 사용한 데이터 정리나 파일정리 스크립트인걸 명시해쥬ㅝ
...
말씀하신 대로, 저장소에 있는 여러 파이썬(.py) 스크립트들이 데이터 전처리 및 파일 관리를 위한 유틸리티 스크립트임을 명시하는 섹션을 추가하겠습니다.

직접 파인튜닝하는 방법 섹션 아래에 해당 내용을 추가하여 전체 README.md 파일을 다시 구성했습니다.
